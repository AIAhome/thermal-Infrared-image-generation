...
if cuda:
    E1 = E1.cuda(opt.local_rank)
    E2 = E2.cuda(opt.local_rank)
    G1 = G1.cuda(opt.local_rank)
    G2 = G2.cuda(opt.local_rank)
    D1 = D1.cuda(opt.local_rank)
    D2 = D2.cuda(opt.local_rank)
    criterion_GAN.cuda()
    criterion_pixel.cuda()
    E1 = torch.nn.parallel.DistributedDataParallel(E1, device_ids=[opt.local_rank], output_device=opt.local_rank)
    E2 = torch.nn.parallel.DistributedDataParallel(E2, device_ids=[opt.local_rank], output_device=opt.local_rank)
    G1 = torch.nn.parallel.DistributedDataParallel(G1, device_ids=[opt.local_rank], output_device=opt.local_rank)
    G2 = torch.nn.parallel.DistributedDataParallel(G2, device_ids=[opt.local_rank], output_device=opt.local_rank)
    D1 = torch.nn.parallel.DistributedDataParallel(D1, device_ids=[opt.local_rank], output_device=opt.local_rank)
    D2 = torch.nn.parallel.DistributedDataParallel(D2, device_ids=[opt.local_rank], output_device=opt.local_rank)
...
train_dataset = ImageDataset("/data1/shaoyuan/%s" % opt.dataset_name, transforms_=transforms_, unaligned=True)
...
# Test data loader
val_dataset = ImageDataset("/data1/shaoyuan/%s" % opt.dataset_name, transforms_=transforms_, unaligned=True, mode="val")
...
def sample_images(batches_done):
    with torch.no_grad():
        ...
        X1 = torch.Tensor(imgs["A"]).cuda(opt.local_rank)
        X2 = torch.Tensor(imgs["B"]).cuda(opt.local_rank)
        _, Z1 = E1(X1)
        _, Z2 = E2(X2)
        fake_X1 = G1(Z2)
        fake_X2 = G2(Z1)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # Set model input
        X1 = torch.Tensor(batch["A"]).cuda(opt.local_rank)
        X2 = torch.Tensor(batch["B"]).cuda(opt.local_rank)

        # Adversarial ground truths
        valid = torch.Tensor(np.ones((X1.size(0), *D1.module.output_shape))).cuda(opt.local_rank)
        fake = torch.Tensor(np.zeros((X1.size(0), *D1.module.output_shape))).cuda(opt.local_rank)
        # -------------------------------
