...
class UNetUp(nn.Module):
    def __init__(self, in_size, out_size, dropout=0.0):
        ...
        layers = [nn.ConvTranspose2d(in_size, out_size, 4, 2, 1), nn.InstanceNorm2d(
...
class GeneratorUNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=1):
        ...
        self.down1 = UNetDown(in_channels, 64, normalize=False)  # channels=3
        ...
        self.final = nn.Sequential(
            nn.Upsample(scale_factor=2), nn.ZeroPad2d((1, 0, 1, 0)), nn.Conv2d(
                128, out_channels, 4, padding=1), nn.Tanh()
        )

    def forward(self, x):
        # U-Net generator with skip connections from encoder to decoder
        d1 = self.down1(x)  # 返回的是一个模型
        d2 = self.down2(d1)
        d3 = self.down3(d2)
        d4 = self.down4(d3)
        d5 = self.down5(d4)
...
class Discriminator(nn.Module):
    def __init__(self, input_shape):
        ...
        # self.output_shape = (1, 256//2**3, 256//2**3)  # TODO 256要改
